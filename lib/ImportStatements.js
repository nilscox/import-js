...
const GROUPINGS_ARRAY = Object.freeze(flattenDeep(STYLES.map((style: string): Array<string> =>
...
export default class ImportStatements {
  ...
  _toGroups(): Array<Array<ImportStatement>> {
    ...
    result.forEach((importStatement: ImportStatement) => {
      const groupIndex = this._getGroupIndex(importStatement);
  ...
  /**
   * Find the group index according to the configuration (if provided),
   * or default to a generic group index based on the import path.
   */
  _getGroupIndex(importStatement: ImportStatement): number {
    const configGroupIndex = this.config.get('groupIndex', {
      pathToImportedModule: importStatement.path,
    });

    if (configGroupIndex) {
      return configGroupIndex;
    }
    ...
    return importStatementGroupIndex(importStatement, packageDependencies, coreModules);
  }
}
